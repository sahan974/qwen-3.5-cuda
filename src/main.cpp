...
const QuantTensor& tensor(const qwen::GgufLoader& l, const std::string& n) {
    auto* t = l.get_tensor(n);
    if (!t) {
        throw std::runtime_error("cannot derive config; missing " + n);
    }
    return *t;
}

bool has(const qwen::GgufLoader& l, const std::string& name) {
    return l.get_tensor(name) != nullptr;
}
...
int derive_base_layer_count(const qwen::GgufLoader& l, int full_attn_interval) {
    ...
    if (count == 0) {
        throw std::runtime_error("could not find a complete base-model layer");
    }
    return count;
}
...
qwen::ModelConfig config_from(const qwen::GgufLoader& l, int ctx) {
    ...
    if (qkv.shape.size() != 2 || z.shape.size() != 2 || conv.shape.size() != 2 ||
        alpha.shape.size() != 2 || gn.shape.size() != 1 ||
        qkv.shape[0] != c.d_model || z.shape[0] != c.d_model ||
        alpha.shape[0] != c.d_model || conv.shape[1] != qkv.shape[1]) {
    ...
    if (z.shape[1] != static_cast<int64_t>(c.gdn_num_v_heads) * c.gdn_value_dim ||
        qkv.shape[1] < z.shape[1] || (qkv.shape[1] - z.shape[1]) % 2 != 0) {
    ...
    if (aq.shape.size() != 2 || ak.shape.size() != 2 || av.shape.size() != 2 ||
        qn.shape.size() != 1 || aq.shape[0] != c.d_model || ak.shape[0] != c.d_model ||
        av.shape != ak.shape || qn.shape[0] <= 0) {
    ...
    if (const auto* sections = l.get_meta_array("qwen35moe.rope.dimension_sections")) {
        ...
        for (const auto& s : *sections) {
            const int part = std::stoi(s);
            if (part < 0) throw std::runtime_error("negative RoPE section");
            sum += part;
        }
        if (sum > 0 && sum <= INT32_MAX / 2) {
            c.partial_rope_dim = static_cast<int>(2 * sum);
        }
    }
    ...
    if (eg.shape.size() != 3 || sg.shape.size() != 2 ||
        eg.shape[0] != c.d_model || sg.shape[0] != c.d_model) {
...
std::string format_chat_prompt(const qwen::GgufLoader& loader, const qwen::BPETokenizer& tokenizer,
    ...
    if (chat_template.find("<|im_start|>") == std::string::npos ||
        chat_template.find("<|im_end|>") == std::string::npos) {
...
int main(int argc, char** argv) {
    ...
    qwen::SamplingConfig sampling;

    try {
        // 1. Argument Parsing
        for (int i = 1; i < argc; ++i) {
            std::string a = argv[i];
            if (a == "--weights" && i + 1 < argc)            path = argv[++i];
            else if (a == "--prompt" && i + 1 < argc)        { prompt = argv[++i]; prompt_set = true; }
            else if (a == "--prompt-file" && i + 1 < argc)   prompt_file = argv[++i];
            else if (a == "--system" && i + 1 < argc)        system = argv[++i];
            else if (a == "--max" && i + 1 < argc)           max_tokens = std::stoi(argv[++i]);
            else if (a == "--ctx" && i + 1 < argc)           ctx_len = std::stoi(argv[++i]);
            else if (a == "--temperature" && i + 1 < argc)   sampling.temperature = std::stof(argv[++i]);
            else if (a == "--top-k" && i + 1 < argc)         sampling.top_k = std::stoi(argv[++i]);
            else if (a == "--top-p" && i + 1 < argc)         sampling.top_p = std::stof(argv[++i]);
            else if (a == "--repeat-penalty" && i + 1 < argc) sampling.repetition_penalty = std::stof(argv[++i]);
            else if (a == "--repeat-last-n" && i + 1 < argc) sampling.repetition_window = std::stoi(argv[++i]);
            else if (a == "--seed" && i + 1 < argc)          sampling.seed = std::stoull(argv[++i]);
            else if (a == "--chat")                          chat = true;
            else if (a == "--benchmark")                     benchmark = true;
            else if (a == "--tokenize-only")                 tokenize_only = true;
            else if (a == "--stream")                        {} // No-op placeholder
            else if (a == "--help") {
                usage(argv[0]);
                return 0;
            } else {
        ...
        if (path.empty()) throw std::runtime_error("--weights is required; there is no fake dry-run mode");
        if (prompt_set && !prompt_file.empty()) throw std::runtime_error("use either --prompt or --prompt-file, not both");
        if (!prompt_file.empty()) prompt = read_text_file(prompt_file);
        if (prompt.empty()) throw std::runtime_error("prompt must not be empty");
        if (!chat && !system.empty()) throw std::runtime_error("--system requires --chat");
        if (max_tokens < 0 || ctx_len <= 0) throw std::runtime_error("invalid token/context limit");
        sampling.validate();

        // 2. Load GGUF Metadata
        const auto gguf_begin = std::chrono::steady_clock::now();
        qwen::GgufLoader loader;
        if (!loader.open(path)) return 1;
        const auto gguf_end = std::chrono::steady_clock::now();

        qwen::ModelConfig cfg = config_from(loader, ctx_len);

        // 3. Initialize Tokenizer & Encode Prompt
        qwen::BPETokenizer tok;
        tok.init(loader);

        if (tok.vocab_size() != cfg.vocab_size) {
            throw std::runtime_error("tokenizer vocabulary and embedding vocabulary differ");
        }

        const std::string model_prompt = chat ? format_chat_prompt(loader, tok, prompt, system) : prompt;
        auto ids = tok.encode(model_prompt, false, chat);

        if (ids.empty()) {
            throw std::runtime_error("prompt encoded to zero tokens");
        }
        if (chat && ids.front() != tok.token_id("<|im_start|>")) {
            throw std::runtime_error("ChatML control tokens were not parsed atomically; check tokenizer.ggml.token_type");
        }
        ...
        if (ids.size() + static_cast<size_t>(max_tokens) > static_cast<size_t>(ctx_len)) {
            throw std::runtime_error("prompt plus generation exceeds --ctx");
        }

        // 4. Load Tensors into VRAM
        VramObserver vram(benchmark);
        const auto tensor_begin = std::chrono::steady_clock::now();
        if (!loader.load_tensors_to_gpu(cfg.n_layers)) {
            throw std::runtime_error("failed to load required GGUF tensors to the GPU");
        }
        const auto tensor_end = std::chrono::steady_clock::now();
        vram.sample();

        // 5. Initialize Model and Sampler
        const auto init_begin = std::chrono::steady_clock::now();
        qwen::CudaContext cuda;
        qwen::QwenModel model;
        model.init(cfg, loader);
        qwen::Sampler sampler(sampling);
        ...
        for (int id : ids) {
            logits = model.decode_logits(id, pos++, cuda);
        }

        const auto prefill_end = std::chrono::steady_clock::now();
        vram.sample();

        // 7. Generation Phase (Decoding)
        std::vector<int> history = ids;
        int generated = 0, decode_steps = 0;
        double decode_compute_ms = 0.0;
        bool stopped_eog = false, first_token_seen = false;
        std::chrono::steady_clock::time_point first_token_time = prefill_end;

        while (generated < max_tokens) {
            int next = sampler.sample(logits, model.vocab_size(), history);
            ...
            logits = model.decode_logits(next, pos++, cuda);
            const auto step_end = std::chrono::steady_clock::now();

            decode_compute_ms += elapsed_ms(step_begin, step_end);
            ++decode_steps;
        }
        ...
        // 8. Output Summary
        const double generation_sec = elapsed_ms(prefill_end, inference_end) / 1000.0;
        std::cout << "\n\nGenerated " << generated << " tokens in " << generation_sec << " s";
        if (generation_sec > 0) {
            std::cout << " (" << generated / generation_sec << " tok/s)";
        }
        std::cout << " [stop: " << (stopped_eog ? "eog" : "max-tokens") << "]\n";

        // 9. Benchmark Reporting
        if (benchmark) {
            const double gguf_ms    = elapsed_ms(gguf_begin, gguf_end);
            const double tensor_ms  = elapsed_ms(tensor_begin, tensor_end);
            const double init_ms    = elapsed_ms(init_begin, init_end);
            const double prefill_ms = elapsed_ms(inference_begin, prefill_end);
            const double decode_ms  = elapsed_ms(prefill_end, inference_end);
            const double total_ms   = elapsed_ms(inference_begin, inference_end);
