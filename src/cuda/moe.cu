...
__global__ void add_kernel(float*out,const float*x,float a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<n)out[i]+=a*x[i];}
void moe_add(float*out,const float*x,float a,int n,cudaStream_t s){add_kernel<<<(n+255)/256,256,0,s>>>(out,x,a,n);CUDA_CHECK(cudaGetLastError());}
__global__ void shared_kernel(float*out,const float*x,const float*gate,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<n)out[i]+=x[i]/(1+expf(-gate[0]));}
void moe_shared_add(float*out,const float*x,const float*g,int n,cudaStream_t s){shared_kernel<<<(n+255)/256,256,0,s>>>(out,x,g,n);CUDA_CHECK(cudaGetLastError());}
}
