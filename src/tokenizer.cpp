...
uint16_t unicode_flags(uint32_t cp) {
    ...
    auto it = std::upper_bound(begin, end, cp,
        [](uint32_t value, const std::pair<uint32_t, uint16_t>& range) {
            return value < range.first;
        }
    );

    uint16_t flags = (it == begin) ? 0 : std::prev(it)->second;
    if (unicode_set_whitespace.count(cp)) {
        flags |= WHITESPACE;
    }
...
// --- Token Chunking ---
// Exact custom pre-tokenizer used by Qwen3.5 in llama.cpp b9222.
// The returned offsets are UTF-8 byte offsets; BPE itself still operates on encoded bytes.
std::vector<size_t> qwen35_chunk_ends(const std::string& text) {
    ...
    for (size_t pos = 0; pos < cps.size();) {
        ...
        if (cp != '\r' && cp != '\n' && !has(f, NUMBER) &&
            ...
            ++pos;
            while (has(flags(pos), LETTER | ACCENT_MARK)) {
                ++pos;
            }
            add(pos);
            continue;
        ...
        // Handle generic whitespace runs
        size_t whitespace = 0;
        ...
        // Default single character fallback
        ++pos;
        add(pos);
    }
    return ends;
...
bool BPETokenizer::init(const GgufLoader& loader) {
    ...
    for (size_t i = 0; i < vocab_.size(); ++i) {
        token_to_id_.emplace(vocab_[i], static_cast<int>(i));
    }

    bpe_ranks_.reserve(merges->size() * 2);
    for (size_t i = 0; i < merges->size(); ++i) {
        bpe_ranks_.emplace((*merges)[i], static_cast<int>(i));
    }
    ...
    if (token_types) {
        for (size_t i = 0; i < token_types->size(); ++i) {
            ...
            if ((type == 2 || type == 3 || type == 4) && !vocab_[i].empty()) {
                special_tokens_.emplace_back(vocab_[i], static_cast<int>(i));
            }
            if (type == 3) {
                control_ids_.insert(static_cast<int>(i));
            }
        }
...
void BPETokenizer::encode_chunk(const std::string& chunk, std::vector<int>& out) const {
    ...
    for (unsigned char c : chunk) {
        symbols.push_back(byte_to_unicode_[c]);
    }
    ...
    while (symbols.size() > 1) {
        ...
        // GPT-2 BPE merges every non-overlapping occurrence of the selected pair in one pass.
    ...
    for (const auto& symbol : symbols) {
        ...
        if (it == token_to_id_.end()) {
            throw std::runtime_error("tokenizer produced a symbol absent from its vocabulary");
        }
...
std::vector<int> BPETokenizer::encode(const std::string& text, bool add_bos, bool parse_special) const {
    ...
    for (size_t pos = 0; pos < text.size();) {
        ...
        for (const auto& special : special_tokens_) {
            if (special.first.size() <= text.size() - pos &&
                ...
                matched = special.second;
                matched_len = special.first.size();
                break;
            }
        }
        ...
        if (pos > plain_begin) {
            encode_plain(text.substr(plain_begin, pos - plain_begin), out);
        }
    ...
    if (plain_begin < text.size()) {
        encode_plain(text.substr(plain_begin), out);
    }
...
std::string BPETokenizer::decode(int id) const {
    if (id < 0 || id >= static_cast<int>(vocab_.size()) || eog_ids_.count(id) || control_ids_.count(id)) {
        return {};
    }

    const std::string& token = vocab_[id];
    if (token.size() >= 3 && token.front() == '<' && token[1] == '|') {
        return token;
    }
    ...
    for (size_t i = 0; i < token.size();) {
        ...
        if (!decode_cp(token, i, cp)) {
            out.push_back(token[before]);
            i = before + 1;
            continue;
        }
        ...
        if (it == unicode_to_byte_.end()) {
            out.append(token, before, i - before);
        } else {
