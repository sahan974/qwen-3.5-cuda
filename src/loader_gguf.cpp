...
T read_scalar(std::ifstream& fin, const char* what) {
    ...
    if (!fin) {
        throw std::runtime_error(std::string("truncated GGUF while reading ") + what);
    }
...
std::string GgufLoader::read_string(std::ifstream& fin) {
    ...
    if (len != 0) {
        fin.read(str.data(), static_cast<std::streamsize>(len));
    }
...
std::vector<std::string> GgufLoader::read_metadata_array(std::ifstream& fin) {
    ...
    if (count > (1ULL << 32)) {
        throw std::runtime_error("invalid GGUF metadata array");
    }

    const auto type = static_cast<GgufMetadataValueType>(raw_type);
    if (type == GgufMetadataValueType::ARRAY) {
    ...
    for (uint64_t i = 0; i < count; ++i) {
        result.push_back(read_metadata_value_as_string(fin, type));
    }
...
void GgufLoader::skip_metadata_value(std::ifstream& fin, GgufMetadataValueType vtype) {
    switch (vtype) {
        ...
        case GgufMetadataValueType::ARRAY: {
            ...
            if (arr_len > (1ULL << 32)) {
                throw std::runtime_error("invalid GGUF metadata array length");
            }
    ...
    if (!fin) {
        throw std::runtime_error("truncated GGUF metadata value");
    }
}
...
bool GgufLoader::open(const std::string& filepath) {
    ...
    for (uint64_t i = 0; i < metadata_count_; ++i) {
        ...
        if (vtype == GgufMetadataValueType::ARRAY) {
            metadata_arrays_[key] = read_metadata_array(fin);
        } else {
            metadata_str_[key] = read_metadata_value_as_string(fin, vtype);
        }
    }
    ...
    for (uint64_t i = 0; i < tensor_count_; ++i) {
        ...
        if (n_dimensions == 0 || n_dimensions > 4) {
            throw std::runtime_error("invalid dimensions for tensor " + tensor.name);
        }
        ...
        if (tensor.size_bytes == 0) {
            throw std::runtime_error("unsupported or malformed tensor '" + tensor.name +
                                     "' (GGML type " + std::to_string(raw_type) + ")");
        }
    ...
    // 4. Validate Payload Offsets & Spans
    ...
    for (const auto& t : tensors_) {
        if (t.offset % alignment != 0 ||
            t.offset > std::numeric_limits<uint64_t>::max() - payload_offset_ ||
            payload_offset_ + t.offset > file_size ||
            t.size_bytes > file_size - (payload_offset_ + t.offset)) {
...
bool GgufLoader::load_tensors_to_gpu(int base_layer_count) {
    ...
    for (const auto& t : tensors_) {
        ...
        if (base_layer_count < 0 || block < 0 || block < base_layer_count) {
            ++selected;
        }
    }
